...
<div class="page-content-wrap">
    ...
    <div class="layui-form" id="table-list">
        <table class="layui-table" lay-even lay-skin="nob">
            ...
            <tbody>
            {volist name="ask_list" id="vo"}
            <tr>
                <!-- <td class="hidden-xs"><input type="checkbox" name="" lay-skin="primary" data-id="1"></td> -->
                <td class="hidden-xs">{$vo.item2}</td>
                <td>{$vo.item0}</td>
                <td>{$vo.item1}</td>
                <td>{$vo.add_time}</td>
                <td><button class="layui-btn layui-btn-mini layui-btn-<?php if($vo['status']==1){echo 'danger';}else{echo 'shua';}?> table-list-status" data-status='{$vo.status}' data-id="{$vo.id}"><?php if($vo['status']==1){echo '未处理';}else{echo '已处理';}?></button></td>
                <td>
                    <div class="layui-inline">
                        <div class="layui-inline">
                            <button class="layui-btn layui-btn-small layui-btn-normal add-btn goBtn" data-url="/index.php/ask/sendSms?id={$vo.id}"><i class="layui-icon">&#xe615;</i></button>
                            <button class="layui-btn layui-btn-small layui-btn-danger delBtn leftBtn"  data-url="/index.php/ask/delete?id={$vo.id}"><i class="layui-icon">&#xe640;</i></button>
                            <button class="layui-btn layui-btn-small layui-btn-warm sendBtn uqu" data-url="/index.php/ask/sendSms?id={$vo.id}" data-id="{$vo.id}"><i class="layui-icon">&#xe609;</i></button>
                        </div>
                    </div>
                </td>
            </tr>
